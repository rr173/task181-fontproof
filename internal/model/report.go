...
// SortConfigDiff makes list-valued comparison output deterministic for both
// the domain API and persisted/exported reports.
func SortConfigDiff(diff *ConfigDiff) {
	sort.Strings(diff.AddedRules)
	sort.Strings(diff.RemovedRules)
	sort.Strings(diff.ChangedFonts)
}
